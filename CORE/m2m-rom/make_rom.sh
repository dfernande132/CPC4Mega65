...
##############################################################################
# CPC4MEGA65 (M4045): Generate osm_const.asm
#
# Line indexes of the on-screen menu (C_MENU_* in mega65.vhd) and the core
# device IDs (C_DEV_CPC_* in globals.vhd), scraped from the VHDL so that the
# firmware never carries a hardcoded menu index. The menu moves on every
# change and a stale index does NOT produce a compile error: it silently
# deselects the wrong line. Same approach as AExp.
##############################################################################
...
awk '/^constant C_MENU_/ {gsub(/;.*/, "", $0); n=$2; sub(/^C_MENU_/, "", n); v=$NF; printf("CPC_OSM_%-18s .EQU %s\n", n, v)}' ../vhdl/mega65.vhd >> osm_const.asm
awk '/^   constant C_DEV_CPC_/ {gsub(/;.*/, "", $0); n=$2; sub(/^C_DEV_CPC_/, "", n); v=$NF; gsub(/x"|"/, "", v); printf("CPC_DEV_%-18s .EQU 0x%s\n", n, v)}' ../vhdl/globals.vhd >> osm_const.asm
