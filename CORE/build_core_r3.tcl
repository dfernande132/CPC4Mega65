# CPC4MEGA65 - build para placa MEGA65 R3.
#
# Es build_core.tcl con UNA linea cambiada: abre CORE-R3.xpr en vez de CORE-R6.xpr. El proyecto
# R3 trae su propio top (top_mega65-r3.vhd) y su propio .xdc; la FPGA es la misma,
# xc7a200tfbg484-2, asi que la lista de ficheros, los file_type y la comprobacion de timing son
# identicos y se dejan tal cual.
#
# OJO: el WNS del R3 NO se hereda del R6. Son restricciones y pinout distintos, y el R3 ha dado
# problemas propios en este proyecto antes (el cuelgue del OSD que reporto su tester). Hay que
# mirar su timing por separado - de eso se encarga la comprobacion del final, que falla la build
# con slack negativo.
#
# En M3001 esto se hizo en un git worktree aparte porque el arbol principal tenia M4 a medias y
# sin commitear. Ahora no hace falta: el R3 tiene que salir EXACTAMENTE del mismo codigo que el
# R6 de la prerelease, que es el que hay aqui.
...
open_project {E:/CPC4MEGA65/core/CORE/CORE-R3.xpr}
...
add_files -norecurse -fileset sources_1 [list \
    ...
    "E:/CPC4MEGA65/core/CORE/vhdl/floppy_copy.vhd" \
]
